...
def chooseRoute(number, option):
    try:
        if option is None:
            ...
            return jsonify({'ERROR': '''PLEASE CHOOSE EVEN, ODD, OR PRIME AS
            AN OPTION OR INPUT A NUMBER AND OMIT THE OPTION PORTION'''}), 400
...
def displayNums(number):
    try:
        if number < 1:
            return jsonify({'ERROR': '''NUMBER MUST BE GREATER
            THAN OR EQUAL TO 1'''}), 400
        elif isinstance(number, int) is False:
            return jsonify({'ERROR': '''PLEASE ENTER A VALID INTEGER GREATER
            THAN OR EQUAL TO 1'''}), 400
